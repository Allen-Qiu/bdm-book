# xgboost with 10-fold cv on traning set  -- 2020.05
...
while(start<=row){
    ...
    model<-xgb.train(data=xgb.DMatrix(pdata.onehot[index.train,],label=label[index.train]), nrounds = 200,objective="binary:logistic",param=params)
    r <- predict(model,  newdata=pdata.onehot[index.test,])
    r <- sapply(r,FUN=function(x){ifelse(x>0.5,1,0)})
    val<-bacc(r,test.label)
    acc<-acc+val
