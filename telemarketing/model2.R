# Random forest with over and under sampling using 10-fold cv on traning set
# -- 2020.05
...
oversampling<-F # False is under-sampling
...
while(start<=row){
    ...
    if(oversampling){
        print('over-sampling')
        index.train.pos<-index.train[pdata$y[index.train]=='yes']
    ...
    rf <- randomForest(y~., data=pdata[index.train.sampling,])
