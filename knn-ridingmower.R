...
library(class)
mydata<-read.table("data/ridingmowers.txt",header=T, sep=" ")
...
par(mfrow=c(2,1))
plot(mydata$Income[owner_idx], mydata$Lot_Size[owner_idx],col='red', xlab='income',ylab='lot size' )
...
plot(train[owner_idx,1], train[owner_idx,2],col='red', xlab='income',ylab='lot size' )
